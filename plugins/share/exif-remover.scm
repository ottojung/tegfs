...
(lambda (config root entry generic-fullpath)
  (define preview-prefix (string-append root "/cache"))
  (define preview? (string-prefix? preview-prefix generic-fullpath))
  (and (not preview?)
       (or
       ...
       (let ()
         ...
         (if (file-exists? path-to-removed)
             path-to-removed
             (let ((status (system* "exiftool" "-all=" "-o" path-to-removed root)))
               (define code (status:exit-val status))
               (unless (= 0 code)
