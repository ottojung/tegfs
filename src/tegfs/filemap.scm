...
(cond-expand
 (guile
  (define-module (tegfs filemap)
    ...
    :use-module ((tegfs sharedinfo) :select (sharedinfo-date sharedinfo-recepientid sharedinfo-senderid sharedinfo-stime sharedinfo?))
...
(define (filemap-set! filemap/2 info)
  ...
  (define date (sharedinfo-date info))
